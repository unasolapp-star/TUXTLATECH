...
/* 4. EXTRACCIÓN Y PAGINACIÓN DEL HISTORIAL */
    ...
    // Construye la cláusula condicional general para filtrar las órdenes según su estado actual
    $where_estado = !empty($filtro_estado) ? " AND estado = '$filtro_estado'" : "";
    // Construye la cláusula condicional con alias para la consulta principal con uniones (JOIN)
    $where_estado_h = !empty($filtro_estado) ? " AND h.estado = '$filtro_estado'" : "";
    ...
    $res_total = mysqli_query($con, "SELECT COUNT(*) as total FROM historial_compras WHERE id_usuario=".$id_usuario . $where_estado);
    ...
    // Ensambla y ejecuta la consulta relacional combinando el catálogo de productos y el historial de compras del usuario
    $result = mysqli_query($con, "SELECT h.fecha_compra,p.id_producto,p.nombre_producto,p.precio_producto,h.cantidad_comprada, h.codigo_orden, h.estado FROM producto as p INNER JOIN historial_compras as h ON p.id_producto=h.id_producto WHERE h.id_usuario=".$id_usuario . $where_estado_h . " ORDER BY h.fecha_compra DESC LIMIT $offset, $registros_por_pagina;");
    ...
    while ($row = mysqli_fetch_array($result)):
        ...
        $codigo_orden = $row['codigo_orden'] ? $row['codigo_orden'] : 'SIN_CODIGO_'.$fecha;
        ...
        if (!isset($historial_agrupado[$codigo_orden])) {
            // Estructura y asocia los metadatos generales de la orden al nuevo índice
            $historial_agrupado[$codigo_orden] = array(
                'fecha' => $fecha,
        ...
        if (!isset($historial_agrupado[$codigo_orden]['productos'][$id_prod])) {
            // Instancia y asocia los detalles elementales del artículo en el arreglo
            $historial_agrupado[$codigo_orden]['productos'][$id_prod] = array(
                "id_producto"=>$id_prod,
        ...
        $historial_agrupado[$codigo_orden]['productos'][$id_prod]['cantidad_comprada'] += $cantidad;
        // Acumula el costo en el total específico de este artículo
        $historial_agrupado[$codigo_orden]['productos'][$id_prod]['total'] += $total;
        
        // Suma el subtotal al monto global de cobro correspondiente a toda la orden
        $historial_agrupado[$codigo_orden]['total_compra'] += $total;
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="myNavbar">
                ...
                <ul class="nav navbar-nav">
                    ...
                    <li><a href="../php/historial_individual.php">Historial</a></li>
                </ul>
...
<body class="container">
    ...
    <h1 style="position: sticky; top: 50px; z-index: 100; background-color: rgba(255, 255, 255, 0.95); padding: 10px; border-radius: 5px; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">Historial de compras</h1>
    ...
    <h3 style="background-color: #f8f9fa; padding: 10px; border-radius: 5px; margin-top: 30px; border-left: 5px solid #337ab7;">
     <b><?= $grupo['fecha'] ?></b> | Total pagado: <span style="color: #5cb85c;">$<?= number_format(floatval($grupo['total_compra']), 2) ?></span> 
     ...
     <div class="pull-right">
         ...
         <a href="estado_compra.php?orden=<?= $grupo['codigo_orden'] ?>" class="btn btn-sm btn-info"><span class="glyphicon glyphicon-eye-open"></span> Estado de compra</a>
         <?php // Muestra el botón de la hoja de envío únicamente si el usuario es administrador
